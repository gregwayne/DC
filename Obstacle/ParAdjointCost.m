function [cst,grad] = AdjointCost(env,LT,NM,ms,psense,gsense,osense,pforward,gforward,oforward,ocdecoder)
    ...
    LossObsFn   = @(osense,nil) env.O.rel_cost*ParFProp(ocdecoder,osense);
    LossAllFn   = @(mot,psense,gsense,osense,nil) (LossPropFn(psense) ...
                                            + LossGoalFn(gsense) ...
                                            + LossObsFn(osense) ...
                                            + LossMotFn(mot));
    % precompute indices
    ...
    for t=1:LT
        ...
        [psenses(:,t+1),pforward]   = ParFProp(pforward,[mot;psenses(:,t)]);
        ...
        %% Goal Model
        [gsenses(:,t+1),gforward]   = ParFProp(gforward,[mot;psenses(:,t);gsenses(:,t)]);
        ...
        %% Obstacle Model        
        [osenses(:,t+1),oforward]   = ParFProp(oforward,[mot;psenses(:,t);osenses(:,t)]);

        % store hidden unit values
        ohids(:,t) = oforward.r1;
        
        % instantaneous gradient
        [~,ocdecoder]=ParFProp(ocdecoder,osenses(:,t+1));
        gradosenses(:,t+1) = env.O.rel_cost*ParBProp(ocdecoder,1);
    ...
    for t=(LT+1):-1:2
        ...
        dbp             = ParBProp(pforward,gradpsenses(:,t));
        dbg             = ParBProp(gforward,gradgsenses(:,t));               
        dbo             = ParBProp(oforward,gradosenses(:,t));
